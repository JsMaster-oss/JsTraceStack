...
class Table:
    ...
    has_id: bool = True                    # False = table de jonction pure
    # Fragment WHERE alternatif, avec un seul %s pour la relation.
...
TABLES: list[Table] = [
    ...
    # SelectionErdt ne porte PAS entityProgramRelationId : son rattachement
    # passe par la jonction SelectionErdt_EntityProgram.
    Table("SelectionErdt",
          scope_sql='"id" IN (SELECT "selectionErdtId" '
                    'FROM public."SelectionErdt_EntityProgram" '
                    'WHERE "entityProgramRelationId" = %s)'),

    Table("SelectionErdt_EntityProgram", entity_col="entityProgramRelationId",
          fks={"entityProgramRelationId": ENTITY_TABLE,
               "selectionErdtId": "SelectionErdt"}),
    ...
    Table("MspCrds", parent=("Logiciels", "logicielId"),
          fks={"logicielId": "Logiciels", "ustID": "UST",
               "selectionEntityId": "SelectionErdt"}),
    ...
    Table("SelectionErdt_Banc", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "bancId": "Bancs"},
          has_id=False),
    Table("SelectionErdt_Environnement", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt",
               "environnementId": "Environnements"}, has_id=False),
    Table("SelectionErdt_GroupeLogiciel", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt",
               "groupeLogicielId": "GroupeLogiciel"}, has_id=False),
    Table("SelectionErdt_GroupeUst", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "groupeUstId": "groupe_ust"},
          has_id=False),
    Table("SelectionErdt_Logiciel", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "logicielId": "Logiciels"},
          has_id=False),
    Table("SelectionErdt_Operateur", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "operateurId": "Operateurs"},
          has_id=False),
    Table("SelectionErdt_Sanction", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "sanctionId": "Sanctions"},
          has_id=False),
    Table("SelectionErdt_TypeTest", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "typeTestId": "TypeTest"},
          has_id=False),
    Table("SelectionErdt_Ust", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt", "ustId": "UST"},
          has_id=False),
    Table("SelectionErdt_TestName", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt"}),
    Table("SelectionErdt_UstSn", parent=("SelectionErdt", "selectionErdtId"),
          fks={"selectionErdtId": "SelectionErdt"}),
]
...
def do_export(dsn: str, relation_id: int, outdir: str,
    ...
    with conn.cursor() as cur:
        ...
        validate_model(cur, strict=not skip_validation)
        ...
        for t in TABLES:
            ...
            if t.scope_sql:
                sql = (f"SELECT {sel} FROM public.{qt(t.name)} "
                       f"WHERE {t.scope_sql}")
...
def validate_model(cur, strict: bool = True) -> None:
    """Confronte le modele du script au schema reel avant toute ecriture.

    Detecte : tables absentes, colonnes FK mal orthographiees, colonnes
    GENERATED non declarees, et surtout les tables scopees oubliees du
    modele (celles qui portent entityProgramRelationId sans etre listees).
    """
    errors: list[str] = []
    warns: list[str] = []
    ...
    modelled = {REAL_TABLE.get(t.name, t.name) for t in TABLES}
    modelled |= {REAL_TABLE.get(g.name, g.name) for g in GLOBALS}

    # 1. Tables du modele absentes de la base.
    for name in sorted(modelled):
        if name not in actual:
            errors.append(f"table absente de la base : {name!r}")

    # 2. Colonnes declarees (FK, scope, generated) reellement presentes ?
    for t in TABLES:
        real = REAL_TABLE.get(t.name, t.name)
        if real not in actual:
            continue
        cols = set(columns_of(cur, t.name))
        for col in t.fks:
            if col not in cols:
                close = [c for c in cols if c.lower().replace("n", "")
                         == col.lower().replace("n", "")]
                hint = f" (proche : {close})" if close else ""
                errors.append(f"{real}.{col} : colonne FK inexistante{hint}")
        if t.entity_col and t.entity_col not in cols:
            errors.append(f"{real}.{t.entity_col} : colonne de scope inexistante")
        if t.parent and t.parent[1] not in cols:
            errors.append(f"{real}.{t.parent[1]} : colonne parente inexistante")
        if t.has_id and "id" not in cols:
            errors.append(f"{real} : declaree has_id=True mais sans colonne id")
        if not t.has_id and "id" in cols:
            warns.append(f"{real} : has_id=False alors qu'une colonne id existe")

    # 3. Colonnes GENERATED non declarees -> INSERT en erreur.
    cur.execute("""
        SELECT table_name, column_name FROM information_schema.columns
        WHERE table_schema = 'public' AND is_generated = 'ALWAYS'
    """)
    declared_gen = {(REAL_TABLE.get(t.name, t.name), g)
                    for t in TABLES for g in t.generated}
    for tbl, col in cur.fetchall():
        if tbl in modelled and (tbl, col) not in declared_gen:
            errors.append(f"{tbl}.{col} : colonne GENERATED non declaree "
                          f"dans generated=(...)")

    # 4. Tables scopees oubliees : le piege qui a coute Sanctions.
    cur.execute("""
        SELECT table_name, column_name FROM information_schema.columns
        WHERE table_schema = 'public'
          AND column_name IN ('entityProgramRelationId',
                              'entity_program_relation_id',
                              'entiteProgrammeRelationId')
    """)
    known_skip = {"Notification_EntityProgram"}
    for tbl, col in cur.fetchall():
        if tbl not in modelled and tbl not in known_skip:
            errors.append(f"{tbl}.{col} : table scopee sur la relation mais "
                          f"ABSENTE du modele -> donnees non migrees")

    # 5. Cles naturelles des globals : unicite reelle ?
    for g in GLOBALS:
        real = REAL_TABLE.get(g.name, g.name)
        if real not in actual or not g.keycols:
            continue
        sel = ", ".join(q(c) for c in g.keycols)
        cur.execute(f"SELECT COUNT(*), COUNT(DISTINCT ({sel})) "
                    f"FROM public.{qt(g.name)}")
        total, distinct = cur.fetchone()
        if total and total != distinct:
            errors.append(f"{real} : cle naturelle {g.keycols} NON unique "
                          f"({total} lignes, {distinct} valeurs) -> "
                          f"resolution ambigue")

    for w in warns:
        print(f"  [avert] {w}")
    if errors:
        print(f"\n  {len(errors)} PROBLEME(S) detecte(s) :")
        for e in errors:
            print(f"   - {e}")
        if strict:
            raise SystemExit(
                "Controle prealable en echec. Corrige le modele en haut du "
                "fichier, ou relance avec --skip-validation en connaissance "
                "de cause."
            )
    else:
        print(f"  == controle prealable OK ({len(modelled)} tables verifiees)")
...
def do_import(dsn: str, indir: str, commit: bool,
              skip_validation: bool = False) -> None:
    ...
    try:
        ...
        for t in TABLES:
            ...
            cols = meta["columns"]
            offset = next_id(cur, t.name) if t.has_id else 0
            local: dict[int, int] = {}
            rows: list[tuple] = []
            unresolved = 0
...
def main() -> None:
    ...
    e.add_argument("--skip-validation", action="store_true")
    ...
    i.add_argument("--skip-validation", action="store_true")

    a = p.parse_args()
    if a.cmd == "export":
        do_export(a.dsn, a.relation_id, a.out, a.skip_validation)
    else:
        do_import(a.dsn, a.indir, a.commit, a.skip_validation)
