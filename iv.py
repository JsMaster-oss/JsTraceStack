...
def build_predecesseur_mapping(lineages, ordre, versions_importees, versions_en_base,
                               mappings):
    """Correspondance version IMPORTÉE → sa version précédente DÉJÀ EN BASE.

    Symétrique de `build_successeur_mapping`. La version précédente n'est pas dans
    IMPORT_DIR (elle est déjà importée), donc ses lignes ne peuvent pas être
    localisées dans un XLSX — mais le tableau donne son identité, et c'est la
    seule information disponible quand celle-ci diffère.

    Attention à ne pas confondre avec « les liens en base font foi » : on écrit
    ici sur la version IMPORTÉE (son parent_id / premier_id), jamais sur la
    version déjà en base, dont les liens ne sont pas touchés.

    Retourne { (version_importée, wono, libellé_norm, famille_norm, produit_norm) :
               {'version', 'wono', 'libelle', 'famille', 'produit'} côté base }.
    """
    m = {}
    for i in range(1, len(ordre)):
        prev_ver, curr_ver = ordre[i - 1], ordre[i]
        if curr_ver not in versions_importees:
            continue
        if prev_ver in versions_importees or prev_ver not in versions_en_base:
            continue
        for lin in lineages:
            blk_prev = lin['versions'].get(prev_ver)
            blk_curr = lin['versions'].get(curr_ver)
            if not blk_prev or not blk_curr:
                continue          # un trou coupe la lignée
            fam = lin.get('famille')
            cle = (curr_ver, resolve_wono(blk_curr['wono'], mappings),
                   _norm(blk_curr['libelle']), _norm(fam or ''),
                   _norm(blk_curr['produit']))
            m[cle] = {'version': prev_ver,
                      'wono': resolve_wono(blk_prev['wono'], mappings),
                      'libelle': blk_prev['libelle'], 'famille': fam,
                      'produit': blk_prev['produit']}
    return m


def build_successeur_mapping(lineages, ordre, succ, versions_importees, mappings):
    """Correspondance SUCCESSEUR → version importée, déduite du tableau.

    Le successeur (ex. V62) est déjà en base, donc absent d'IMPORT_DIR : ses
    lignes ne peuvent pas être localisées dans un XLSX. Mais le tableau donne son
    identité, et c'est la seule information disponible quand celle-ci diffère de
    la dernière version importée — sans quoi `rattachement_successeur` n'apparie
    rien et le lot importé reste orphelin de V62.

    Retourne { (wono, libellé_norm, famille_norm, produit_norm) du SUCCESSEUR :
               {'version', 'wono', 'libelle', 'famille', 'produit'} côté import }.
    """
    if succ not in ordre:
        return {}
    i = ordre.index(succ)
    if i == 0:
        return {}
    precedente = ordre[i - 1]          # colonne juste avant : un trou coupe la lignée
    if precedente not in versions_importees:
        return {}

    m = {}
    for lin in lineages:
        blk_succ = lin['versions'].get(succ)
        blk_imp = lin['versions'].get(precedente)
        if not blk_succ or not blk_imp:
            continue
        fam = lin.get('famille')
        cle = (resolve_wono(blk_succ['wono'], mappings), _norm(blk_succ['libelle']),
               _norm(fam or ''), _norm(blk_succ['produit']))
        m[cle] = {'version': precedente, 'wono': resolve_wono(blk_imp['wono'], mappings),
                  'libelle': blk_imp['libelle'], 'famille': fam,
                  'produit': blk_imp['produit']}
    return m
...
def check_g_mapping_manuel(xlsx_data, lineages, ordre, db_data, mappings, issues, proposals):
    ...
    succ = (determine_successor(db_data, max(xlsx_data, key=version_sort_key))
        ...
        for ver in ordre:
            ...
            if ver not in indexes:
                if ver == succ:
                    proposals.append({
                        'type': 'info',
                        'message': (f"[MAPPING l.{row_no}] version '{ver}' = successeur figé : "
                                    f"son bloc sert au rattachement final, pas au chaînage "
                                    f"entre versions importées."),
                    })
                elif ver in db_data['versions']:
                    proposals.append({
                        'type': 'info',
                        'message': (f"[MAPPING l.{row_no}] version '{ver}' déjà en base : "
                                    f"ses propres liens ne sont pas touchés, mais son bloc "
                                    f"sert à rattacher la version importée qui la suit."),
                    })
                else:
                    proposals.append({
                        'type': 'info',
...
def _contrat_precedent(prev_imported, cible):
    """Contrat de la version précédente désigné par le tableau, ou None.

    Recherche exacte puis, à défaut, insensible à la casse et aux espaces : les
    libellés saisis à la main dans le tableau ne sont pas toujours au caractère près.
    """
    direct = prev_imported.get((cible['wono'], cible['libelle']))
    if direct:
        return direct
    for (w, lib), c in prev_imported.items():
        if w == cible['wono'] and _norm(lib) == _norm(cible['libelle']):
            return c
    return None


def import_single_version(conn, version_name, version_data, db_data, mappings,
                           prev_imported, system_user_id, pred_map=None):
    """
    ...
    for (wono, libelle), familles_rows in contrats_raw.items():
        ...
        # Version précédente déjà en base : son identité n'est pas dans les XLSX,
        # seul le tableau peut relier les deux.
        if prev_contrat is None and pred_map:
            for famille, rows in familles_rows.items():
                for r in rows:
                    cible = (pred_map or {}).get(
                        (version_name, wono, _norm(r['libelle']),
                         _norm(famille), _norm(r['nom_produit'])))
                    if not cible:
                        continue
                    prev_contrat = _contrat_precedent(prev_imported, cible)
                    if prev_contrat:
                        log.info("Chaînage via le tableau : %s (%s, %s) → %s (%s, %s)",
                                 version_name, wono, libelle, cible['version'],
                                 cible['wono'], cible['libelle'])
                        break
                if prev_contrat:
                    break
        ...
        for famille, rows in familles_rows.items():
            ...
            # --- Temps 2ter : version précédente déjà en base ---
            for r in resolved:
                if r['prev'] is not None or not pred_map:
                    continue
                cible = pred_map.get(
                    (version_name, wono, _norm(r['raw_libelle']),
                     _norm(famille), _norm(r['raw'][1])))
                if not cible:
                    continue
                vises = [l for k, l in prev_lignes.items()
                         if _norm(k[1]) == _norm(cible['produit'])
                         and l is not None and l not in [x['prev'] for x in resolved]]
                if len(vises) == 1:
                    r['prev'] = vises[0]
                    stats['liens_manuels'] += 1
                elif len(vises) > 1:
                    log.warning("Chaînage via le tableau : le produit '%s' de %s désigne "
                                "%d lignes en %s — lien non établi",
                                cible['produit'], version_name, len(vises), cible['version'])
...
def rattachement_successeur(conn, db_data, all_imported, succ_map=None):
    ...
    # Garde-fou : le ré-ancrage cible un premier_id ENTIER, et depuis que le lot
    # importé peut hériter d'une racine préexistante (chaînage vers une version
    # déjà en base), cet UPDATE pourrait atteindre l'En cours. On l'exclut
    # explicitement — l'invariant prime sur l'unification des racines.
    cur.execute("SELECT id FROM version WHERE nom_version = 'En cours'")
    row_ec = cur.fetchone()
    id_en_cours = row_ec['id'] if row_ec else -1
    ...
    succ_map = succ_map or {}

    def cible_manuelle(fam, produit):
        """Ce que le tableau dit de cette ligne du successeur, ou None."""
        return succ_map.get((wono, _norm(libelle), _norm(fam or ''), _norm(produit)))
    ...
    for (wono, libelle), sc in succ_data.items():
        ...
        # Le libellé (ou le wono) a changé entre l'import et le successeur :
        # l'identité ne suffit pas, on suit le tableau.
        if imp_contrat is None:
            for fam, lignes in sc['lignes'].items():
                for (_a, prod, _s) in lignes:
                    cible = cible_manuelle(fam, prod)
                    if not cible:
                        continue
                    imp_contrat = all_imported.get(cible['version'], {}).get(
                        (cible['wono'], cible['libelle']))
                    if imp_contrat:
                        log.info("Rattachement via le tableau : successeur (%s, %s) → "
                                 "%s (%s, %s)", wono, libelle, cible['version'],
                                 cible['wono'], cible['libelle'])
                        break
                if imp_contrat:
                    break
        ...
        if sc.get('premier_id') is not None:
            ...
            if imp_contrat['premier_id'] != sc['premier_id']:
                cur.execute("""
                    UPDATE contrat SET premier_id=%s
                    WHERE premier_id=%s AND id_version <> %s
                """, (sc['premier_id'], imp_contrat['premier_id'], id_en_cours))
        ...
        for fam, sc_lignes in sc['lignes'].items():
            imp_lignes = imp_contrat['lignes'].get(fam, {})
            matches = match_lines(sc_lignes, imp_lignes)
            for ident, sline in sc_lignes.items():
                imp_line = matches.get(ident)

                # Le tableau prime quand l'identité n'a pas suffi : c'est le seul
                # moyen de rattacher un successeur dont le produit a changé.
                if imp_line is None:
                    cible = cible_manuelle(fam, ident[1])
                    if cible:
                        vises = [l for k, l in imp_lignes.items()
                                 if _norm(k[1]) == _norm(cible['produit'])]
                        if len(vises) == 1:
                            imp_line = vises[0]
                        elif len(vises) > 1:
                            log.warning("Rattachement : le tableau désigne le produit '%s' "
                                        "mais %d lignes y correspondent (%s, %s) — ignoré",
                                        cible['produit'], len(vises), wono, libelle)

                paires.append((ident, sline, imp_line))
        ...
        for ident, sline, imp_line in paires:
            ...
            if sline['premier_id'] is not None and imp_line['premier_id'] != sline['premier_id']:
                cur.execute("""
                    UPDATE contrat_ligne cl SET premier_id=%s
                    FROM contrat c
                    WHERE cl.id_contrat = c.id
                      AND cl.premier_id = %s AND c.id_version <> %s
                """, (sline['premier_id'], imp_line['premier_id'], id_en_cours))
            ...
            for date_mad, s_ech in sline['echeances'].items():
                ...
                if s_ech['premier_id'] is not None and imp_ech['premier_id'] != s_ech['premier_id']:
                    cur.execute("""
                        UPDATE echeance e SET premier_id=%s
                        FROM contrat_ligne cl JOIN contrat c ON cl.id_contrat = c.id
                        WHERE e.id_contrat_ligne = cl.id
                          AND e.premier_id = %s AND c.id_version <> %s
                    """, (s_ech['premier_id'], imp_ech['premier_id'], id_en_cours))
            rattached += 1
...
def phase1(xlsx_data, db_data, mappings):
    ...
    liens_manuels, liens_exacts = check_g_mapping_manuel(
    ...
    if MAPPING_XLSX.exists():
        ...
        print(f"  {len(lineages)} lignée(s) lue(s) : {liens_manuels} lien(s) établi(s) "
              f"par le tableau, {liens_exacts} déjà couvert(s) par correspondance exacte "
              f"(lien créé automatiquement à l'import).")
    ...
    if lineages:
        ...
        REPORT.add('Phase 1 — Mapping manuel', 'info',
                   ...
                   f"{liens_manuels} lien(s) établi(s) par le tableau · "
                   f"{liens_exacts} paire(s) déjà couverte(s) par correspondance exacte "
                   f"(le lien est créé automatiquement à l'import, le tableau n'a rien "
                   f"à y ajouter)")
    ...
    if blocking:
        ...
        return False, mappings, encours_targets, lineages, ordre
    ...
    return True, mappings, encours_targets, lineages, ordre

# ---------------------------------------------------------------------------
# PHASE 2 orchestration
# ---------------------------------------------------------------------------

def phase2(xlsx_data, db_data, mappings, encours_targets, lineages=None, ordre=None):
    print("\n" + "=" * 60)
    ...
    pred_map = build_predecesseur_mapping(
        lineages or [], ordre or [], set(versions), set(archived_in_db), mappings)
    if pred_map:
        log.info("Chaînage : %d correspondance(s) vers une version déjà en base, "
                 "issues du tableau.", len(pred_map))
    ...
    try:
        for version_name in versions:
            ...
            try:
                ...
                imported, stats = import_single_version(
                    ...
                    db_data, mappings, prev_imported, system_user_id, pred_map,
                )
                conn.commit()
        ...
        if all_imported:
            succ = determine_successor(db_data, max(all_imported, key=version_sort_key))
            succ_map = build_successeur_mapping(
                lineages or [], ordre or [], succ, set(all_imported), mappings)
            if succ_map:
                log.info("Rattachement : %d correspondance(s) successeur issues du tableau.",
                         len(succ_map))
            rattachement_successeur(conn, db_data, all_imported, succ_map)
...
def main():
    ...
    ok, mappings, encours_targets, lineages, ordre = phase1(xlsx_data, db_data, mappings)
    if not ok:
        sys.exit(1)
    ...
    phase2(xlsx_data, db_data, mappings, encours_targets, lineages, ordre)
