...
_MAPPING_ROLES = {
    'produit': 'produit',
    'contrat': 'libelle', 'libelle': 'libelle', 'libellecontrat': 'libelle',
    'wono': 'wono', 'numwono': 'wono',
}
...
def load_manual_mapping():
    ...
    lineages : [ {'_row': int, 'versions': {version: {'produit','libelle','wono'}}} ]
    ...
    for bi, (ver, ci) in enumerate(starts):
        ...
        for col in range(ci, end):
            role = _MAPPING_ROLES.get(_norm(rows[1][col]).replace(' ', ''))
            if role:
                cols[role] = col
    ...
    for ri, row in enumerate(rows[2:], start=3):
        ...
        for ver in ordre:
            ...
            if any(blk.values()):
                versions[ver] = blk
        if versions:
            lineages.append({'_row': ri, 'versions': versions})

    log.info("Tableau de mapping manuel : %d lignée(s) sur %s",
...
def build_manual_lookup(version_data, mappings):
    ...
    for famille, rows in version_data.items():
        for row in rows:
            ...
            if (ck, ident) in seen:      # une ligne = N échéances → dédoublonner
                continue
            seen.add((ck, ident))
...
def chain_key(version, wono_raw, libelle, activite, produit, scenario):
    """Clé de mappings['chaine'] — valeurs BRUTES du XLSX, comme partout ailleurs."""
    return f"{version}|{wono_raw}|{libelle}|{activite}|{produit}|{scenario}"
...
def load_imported_version_from_db(conn, version_name):
    ...
    cur.execute("""
        SELECT c.id, c.num_wono, c.libelle, c.premier_id,
               ...
               p.nom_produit, a.nom_activite, s.nom_scenario,
               e.id AS ech_id, e.date_mad, e.premier_id AS ech_premier_id
        FROM version v
    ...
    for row in rows:
        ...
        if row['cl_id']:
            lk = (row['nom_activite'], row['nom_produit'], row['nom_scenario'])
            if lk not in imported[ck]['lignes']:
                imported[ck]['lignes'][lk] = {
                    'id': row['cl_id'],
            ...
            if row['ech_id']:
                imported[ck]['lignes'][lk]['echeances'][row['date_mad']] = {
                    'id': row['ech_id'],
...
def check_g_mapping_manuel(xlsx_data, lineages, ordre, mappings, issues, proposals):
    ...
    for lin in lineages:
        ...
        for i in range(1, len(ordre)):
            ...
            for cl in curr_c:
                ck = chain_key(curr_ver, cl['wono_raw'], cl['libelle'], *cl['raw'])
...
def import_single_version(conn, version_name, version_data, db_data, mappings,
    ...
    for (wono, libelle), familles_rows in contrats_raw.items():
            ...
            for famille, rows in familles_rows.items():
                for r in rows:
                    ...
                    cm = mappings['chaine'].get(chain_key(
                        version_name, r['num_wono'], r['libelle'],
        ...
        for famille, rows in familles_rows.items():
            ...
            prev_lignes = prev_contrat['lignes'] if prev_contrat else {}
            ...
            for r in resolved:
                cm = mappings['chaine'].get(chain_key(
                    version_name, r['raw_wono'], r['raw_libelle'], *r['raw']))
                if not cm:
                    continue
            ...
            for r in resolved:
                ...
                imported[(wono, libelle)]['lignes'][lk] = {
                ...
                for date_mad, agg in ech_par_date.items():
                    ...
                    if e_premier is None:
                        cur.execute("UPDATE echeance SET premier_id=%s WHERE id=%s",
                                    (ech_id, ech_id))
                        e_premier = ech_id
                    imported[(wono, libelle)]['lignes'][lk]['echeances'][date_mad] = {
...
def rattachement_successeur(conn, db_data, all_imported):
    ...
    for (wono, libelle), sc in succ_data.items():
        ...
        matches = match_lines(sc['lignes'], imp_contrat['lignes'])
        for ident, sline in sc['lignes'].items():
